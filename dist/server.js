#!/usr/bin/env node
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const ApiController_1 = require("./api/ApiController");
// Fonction principale pour traiter les commandes
async function main() {
    const args = process.argv.slice(2);
    const command = args[0];
    try {
        let result;
        switch (command) {
            case 'login':
                const loginData = JSON.parse(args[1] || '{}');
                result = await ApiController_1.ApiController.login(loginData);
                break;
            case 'validate-token':
                if (!args[1]) {
                    result = { statut: 'erreur', message: 'Token requis' };
                }
                else {
                    result = await ApiController_1.ApiController.validateToken(args[1]);
                }
                break;
            case 'logout':
                result = await ApiController_1.ApiController.logout();
                break;
            case 'create-user':
                const userData = JSON.parse(args[1] || '{}');
                result = await ApiController_1.ApiController.createUser(userData);
                break;
            case 'suivi':
                if (!args[1]) {
                    result = { statut: 'erreur', message: 'Code de suivi requis' };
                }
                else {
                    result = await ApiController_1.ApiController.suivreColis(args[1]);
                }
                break;
            case 'creer-cargaison':
                const cargaisonData = JSON.parse(args[1] || '{}');
                result = await ApiController_1.ApiController.creerCargaison(cargaisonData);
                break;
            case 'ajouter-colis':
                const colisData = JSON.parse(args[1] || '{}');
                result = await ApiController_1.ApiController.ajouterColis(colisData);
                break;
            case 'get-cargaisons':
                result = await ApiController_1.ApiController.getAllCargaisons();
                break;
            case 'get-colis':
                result = await ApiController_1.ApiController.getAllColis();
                break;
            case 'changer-etat':
                if (!args[1] || !args[2]) {
                    result = { statut: 'erreur', message: 'Code et nouvel état requis' };
                }
                else {
                    result = await ApiController_1.ApiController.changerEtatColis(args[1], args[2]);
                }
                break;
            default:
                result = {
                    statut: 'erreur',
                    message: 'Commande non reconnue',
                    commands: [
                        'login <json>',
                        'validate-token <token>',
                        'logout',
                        'create-user <json>',
                        'suivi <code>',
                        'creer-cargaison <json>',
                        'ajouter-colis <json>',
                        'get-cargaisons',
                        'get-colis',
                        'changer-etat <code> <etat>'
                    ]
                };
        }
        // Retourner le résultat en JSON
        console.log(JSON.stringify(result));
    }
    catch (error) {
        console.log(JSON.stringify({
            statut: 'erreur',
            message: 'Erreur serveur: ' + (error instanceof Error ? error.message : 'Erreur inconnue')
        }));
        process.exit(1);
    }
}
// Exécuter la fonction principale
main();
//# sourceMappingURL=server.js.map